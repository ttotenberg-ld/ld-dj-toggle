...
Pattern.prototype.pitchwheel = function (options = {}) {
  let { ctx, id = 1 } = options;
  return this.tag(id).onPaint((painterCtx, time, haps) =>
    pitchwheel({
      ...options,
      time,
      // Use ctx from options if provided, otherwise use the painter's ctx (inline canvas),
      // falling back to fullscreen overlay if neither is available
      ctx: ctx || painterCtx || getDrawContext(),
